...
class Program():
    def GET(self, day):
        ...
        if len(data) == 0:
            return render.program(data=None, day=day,
    ...
    def POST(self, day):
        ...
        for rawHour, temp in data.items():
            ...
            if len(result) == 0:  # There is no data in the DB
                insertSql = """
                    INSERT INTO program (weekDay,hour,targetTEmp) VALUES(?,?,?)
                    """
                ...
                updateSql = """
                UPDATE program SET targetTemp = ? WHERE weekDay = ?
                AND hour = ?
                """
                ...
                if cur.rowcount != 1:
                    ...
                    return render.program(data=None, day=day,
                                          domain=web.ctx.homedomain)
        conn.commit()
        return render.program(data=self.__getWeekDays(day), day=day,
                              domain=web.ctx.homedomain)

    def __getWeekDays(self, day):
        num = self.__weekDayString2Num(day)
        sql = """
        SELECT hour, targetTemp FROM program WHERE weekDay = ? ORDER BY hour
        """
        conn = sqlite3.connect("thermostat.db")
        cur = conn.cursor()
        cur.execute(sql, (num,))
