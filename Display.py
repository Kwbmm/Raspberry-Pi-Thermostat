...
class DisplayDevice:
    ...
    def __init__(self, displayAddress, thermostatSensor, targetTemp,
                 controller, displayWidth=20):
        ...
        self._lcdString("Env Temp: Fetching...", self.LCD_LINE_1)
        self._lcdString("Target Temp: " + str(self.targetTemp),
                        self.LCD_LINE_2)

        dispatcher.connect(self.updateEnvTempScreen,
                           signal=self.thermostatSensor.THERMOSTAT_TO_DISPLAY_SIG,
                           sender=self.thermostatSensor)
        dispatcher.connect(self.updateTargetTempScreen,
                           signal=self.controller.BTN_UP_TO_DISPLAY_SIG,
                           sender=self.controller)
        dispatcher.connect(self.updateTargetTempScreen,
                           signal=self.controller.BTN_DOWN_TO_DISPLAY_SIG,
                           sender=self.controller)
